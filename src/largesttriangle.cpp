...
inline void solve() {
    ...
    n = sz(arr);
    double res = 0;
    auto f = [&](int i, int j, int mid) -> double {
        return segDist(arr[i], arr[j], arr[mid]);
    };
    rep(i, 0, n) {
        rep(j, i + 2, n) {
            int lo = i + 1, hi = j - 1;
            while (hi - lo > 3) {
                int a = lo + hi >> 1, b = a + 1;
                if (f(i, j, a) > f(i, j, b))hi = b - 1;
                else lo = a + 1;
            }
            rep(k, lo, hi + 1) {
                res = max(res, arr[i].cross(arr[k]) + arr[k].cross(arr[j]) + arr[j].cross(arr[i]));
            }
        }
    }
    cout << res / 2;
