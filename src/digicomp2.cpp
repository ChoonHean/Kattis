...
inline void solve() {
    ...
    while (!q.empty()) {
        ...
        q.pop();
        if (adj[i].first != -1) {
            a[adj[i].first] += a[i] / 2 + ((a[i] & 1) && res[i] == 'L');
            if (!--deg[adj[i].first])q.push(adj[i].first);
        }
        if (adj[i].second != -1) {
            a[adj[i].second] += a[i] / 2 + ((a[i] & 1) && res[i] == 'R');
            if (!--deg[adj[i].second])q.push(adj[i].second);
        }
        if (a[i] & 1)res[i] = res[i] == 'L' ? 'R' : 'L';
    }
    cout << res;
}
