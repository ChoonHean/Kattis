...
inline void solve() {
    int n, k, w;
    cin >> n >> k;
    min_cost_max_flow mf(2 * n + 5);
    //company n,s 2n+3, t 2n+4
    rep(i, 0, n) {//company to locations
        cin >> w;
        mf.add_edge(n, n + i + 1, 1, w);
    }
    rep(i, 0, n)
        rep(j, i + 1, n) {//location to location
            cin >> w;
            mf.add_edge(i, n + j + 1, 1, w);
        }
    mf.add_edge(2 * n + 3, n, k, 0);//src to company
    rep(i, 0, n)mf.add_edge(2 * n + 3, i, 1, 0);//src to location
    rep(i, 0, n)mf.add_edge(n + i + 1, 2 * n + 4, 1, 0);//location to sink
    cout << mf.mcmf(2 * n + 3, 2 * n + 4).second;
}
