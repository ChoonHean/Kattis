...
inline void solve() {
    ...
    cin >> n;
    read(n);
    vector<hmap<int, int>> arr;
    rep(i, 0, n)arr.pb(primeFactors(a[i]));
    hmap<int, int> h;
    for (const auto &mp: arr)for (const auto &[x, y]: mp)h[x] += y;
    vpii res;
    int g = 1, t = 0;
    for (const auto &[x, y]: h)if (y >= n)res.eb(x, y / n), g *= pow(x, y / n);
    for (auto &mp: arr) {
        for (const auto &[x, y]: res)t += max(y - mp[x], 0);
    }
    pr(g, t);
}
