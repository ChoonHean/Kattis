...
inline void solve() {
    ...
    rep(i, 0, g)cin >> x >> y, a[x].insert(y);
    cin >> m;
    rep(i, 0, m) {
        cin >> x >> y >> r;
        rep(j, max(0, x - r), min(10001, x + r + 1)) {
            int mx = sqrt(r * r - pow(x - j, 2));
            auto it = a[j].lower_bound(y - mx);
            while (it != a[j].end() && *it <= y + mx)it = a[j].erase(it);
        }
    }
    int res = 0;
    for (const auto &s: a)res += sz(s);
    cout << res;
}
