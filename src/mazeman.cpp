...
inline void solve() {
    ...
    rep(i, 0, n)
        rep(j, 0, m) {
            if (arr[i][j] != 'X') {
                int cur = i * m + j;
                if (arr[i][j] == '.')uf.dots[cur]++;
                else if (arr[i][j] != ' ')uf.entry[cur] = 1;
                if (i && arr[i - 1][j] != 'X' && !(isalpha(arr[i - 1][j]) && isalpha(arr[i][j]))) {
                    uf.unionset((i - 1) * m + j, cur);
                }
                if (j && arr[i][j - 1] != 'X' && !(isalpha(arr[i][j - 1]) && isalpha(arr[i][j]))) {
                    uf.unionset(i * m + j - 1, cur);
                }
            }
        }
    int res1 = 0, res2 = 0;
