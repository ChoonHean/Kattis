...
vector<vector<bitset<26>>> used(26, vector<bitset<26>>(26));
vvi arr;
string res = "   ", s;

bool h(int pos) {
    int first = inf;
    auto &cur = used[res[0]][res[1]];
    rep(0, 26) {
        if (!cur[i]) {
            auto it = upper_bound(all(arr[i]), pos);
            if (it != arr[i].end()) {
                if (*it < first) {
                    res[2] = i;
                    first = *it;
...
bool g(int pos) {
    ...
    rep(0, 26) {
        auto it = upper_bound(all(arr[i]), pos);
        if (it != arr[i].end())tree.insert({*it, i});
    }
    for (auto [i, c]: tree) {
        res[1] = c;
        if (h(i))return true;
...
void f() {
    set<pii> tree;
    rep(0, 26) {
        if (!arr[i].empty())tree.insert({arr[i][0], i});
    }
    for (auto [i, c]: tree) {
        res[0] = c;
        if (g(i))return;
