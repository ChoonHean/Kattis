...
inline void solve() {
    ...
    auto f = [&](auto &self, int n) -> vi {
        ...
        rep(1, (1 << l) - 1) {
            ...
            if (cur[0] == '0')continue;
            if (cur == "1")continue;
            if (n % stoi(cur))continue;
            cur = "";
            for (int j = ((1 << l) - 1) ^ i; j; j -= lsb(j))cur.pb(s[ctz(lsb(j))]);
            vi ret;
            if (cur.empty())ret = {0};
            else ret = self(self, stoi(cur));
            if (sz(ret) > sz(res))res = ret;
            else if (sz(ret) == sz(res))res = min(res, ret);
        }
