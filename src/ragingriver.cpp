...
inline void solve() {
    int p, n, l, u, v;
    cin >> p >> n >> l;
    min_cost_max_flow mf(n + 3);
    rep(i, 0, l) {
        cin >> u >> v;
        if (v < u)swap(v, u);
        if (u == -2) {
            if (v == -1)mf.add_edge(n, n + 1, 1, 1, 0);
            else mf.add_edge(n, v, 1, 1, 0);
        } else if (u == -1)mf.add_edge(n + 1, v, 1, 1, 0);
        else mf.add_edge(u, v, 1, 1, 0);
    }
    mf.add_edge(n + 2, n, p, 0);
    auto ret = mf.mcmf(n + 2, n + 1);
    if (ret.first < p)cout << p - ret.first << " people left behind";
