...
inline void solve() {
    ...
    rep(i, 0, n) {
        ...
        else u = mp[s] = cnt++, adj.eb(), tr.eb(), a.pb(s);
        if (mp.contains(t))v = mp[t];
        else v = mp[t] = cnt++, adj.eb(), tr.eb(), a.pb(t);
        adj[u].eb(v);
        tr[v].eb(u);
    }
    n = cnt;
    vi scc = kosaraju(adj, tr);
    vs lone;
    vvi g(*max_element(all(scc)) + 1);
    vector<vs> res;
    rep(i, 0, n)g[scc[i]].pb(i);
    for (const auto &v: g) {
        if (sz(v) == 1)lone.pb(a[v[0]]);
        else {
            res.eb();
            for (const int &j: v)res.back().pb(a[j]);
        }
    }
    for (auto &v: res)sort(all(v));
    sort(all(res));
    sort(all(lone));
    for (const auto &v: res)pr("okay"), pr(v);
    if (sz(lone))pr("avoid");
    for (const auto &s: lone)pr(s);
}
