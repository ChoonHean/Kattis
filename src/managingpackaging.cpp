...
inline void solve() {
    ...
    while (cin >> n) {
        ...
        hmap<string, vs> adj;
        hmap<string, int> deg;
        PQ<string, vs, greater<>> pq;
        rep(i, 0, n) {
            getline(cin, s);
            vs cur = split(s, ' ');
            rep(j, 1, sz(cur))adj[cur[j]].pb(cur[0]);
            if (sz(cur) == 1)pq.push(cur[0]);
            else deg[cur[0]] = sz(cur) - 1;
        }
        vs res;
        while (!pq.empty()) {
            string cur = pq.top();
            pq.pop();
            res.pb(cur);
            for (const string &t: adj[cur])if (!--deg[t])pq.push(t);
        }
        if (sz(res) == n)for (const string &t: res)pnl(t);
        else pnl("cannot be ordered");
        cout << nl;
    }
}
