...
inline void solve() {
    ...
    rep(i, 0, n)prefa[i + 1] = prefa[i] + a[i], prefb[i + 1] = prefb[i] + b[i];
    vector<P> arr;
    rep(i, 0, n + 1)arr.eb(prefa[i], prefb[i]);
    auto ret = closest(arr);
    cout << (ret.first - ret.second).dist2();
}
