...
inline void solve() {
    ...
    while (getline(cin, s)) {
        ...
        vi pre(n);
        pre[0] = count(arr[0].begin(), arr[0].end(), '*');
        rep(i, 1, n)pre[i] = pre[i - 1] + count(all(arr[i]), '*');
        cout << string(m - pre[0], '.') << string(pre[0], '*') << nl;
        rep(i, 1, n) {
            cout << string(m - pre[i], '.') << string(pre[i] - pre[i - 1], '*') << string(pre[i - 1], '.') << nl;
        }
        cout << nl;
    }
}
