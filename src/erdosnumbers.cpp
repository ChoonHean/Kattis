...
inline void solve() {
    ...
    while (getline(cin, s)) {
        ...
        else u = mp[cur[0]] = sz(mp), adj.eb();
        rep(i, 1, sz(cur)) {
            if (mp.contains(cur[i]))v = mp[cur[i]];
            else v = mp[cur[i]] = sz(mp), adj.eb();
            adj[u].eb(v, 1);
            adj[v].eb(u, 1);
        }
    }
    vi dist = dijkstra(adj, mp["PAUL_ERDOS"]);
    for (const string &s: a) {
        int res = dist[mp[s]];
        if (res == INT_MAX)pr(s, "no-connection");
        else pr(s, res);
    }
}
