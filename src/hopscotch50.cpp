...
inline void solve() {
    ...
    vvi a(n, vi(n));
    vvpii num(n * n + 1);
    rep(i, 0, n)rep(j, 0, n)cin >> a[i][j], num[a[i][j]].eb(i, j);
    vvi dp(n, vi(n, inf));
    rep(i, 0, n)rep(j, 0, n)if (a[i][j] == 1)dp[i][j] = 0;
    for (int t = 2; t <= m; t++)
        for (const auto &[i, j]: num[t - 1])
            for (const auto &[k, l]: num[t])
                dp[k][l] = min(dp[k][l], dp[i][j] + abs(i - k) + abs(j - l));
    int res = inf;
    rep(i, 0, n)rep(j, 0, n)if (a[i][j] == m)res = min(res, dp[i][j]);
    cout << (res == inf ? -1 : res);
}
