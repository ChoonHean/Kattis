...
inline void solve() {
    ...
    rep(i, 0, n) {
        if (s[i] == '(')st.push(i);
        else if (s[i] == ')')f.pb(st.top()), b.pb(i), st.pop();
    };
    rep(i, 1, 1 << sz(f)) {
        hset<int> rm;
        rep(j, 0, sz(f))if ((i >> j) & 1)rm.insert(f[j]), rm.insert(b[j]);
        string ans;
        rep(j, 0, n)if (!rm.contains(j))ans.pb(s[j]);
        res.pb(ans);
    }
    sort(all(res));
    auto end = unique(all(res));
    for (auto it = res.begin(); it != end; it++)pnl(*it);
}
