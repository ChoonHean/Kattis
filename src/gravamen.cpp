...
inline void solve() {
    ...
    while (lo < hi) {
        ...
        rep(i, 0, l)mf.add_edge(N + i, a[i].first, 1), mf.add_edge(N + i, a[i].second, 1), mf.add_edge(M, N + i, 1);
        rep(i, 0, N)mf.add_edge(i, M + 1, mid);
        if (mf.dinic(M, M + 1) == l)hi = mid;
        else lo = mid + 1;
    }
    mf.EL.clear();
    for (auto &v: mf.AL)v.clear();
    rep(i, 0, l)mf.add_edge(N + i, a[i].first, 1), mf.add_edge(N + i, a[i].second, 1), mf.add_edge(M, N + i, 1);
    rep(i, 0, N)mf.add_edge(i, M + 1, lo);
    mf.dinic(M, M + 1);
    rep(i, N, M) {
        const auto &[v, cap, flow] = mf.EL[mf.AL[i][0]];
        if (flow == 1)pr("INDV", a[i - N].first + 1);
        else pr("CORP", a[i - N].second - r + 1);
    }
}
