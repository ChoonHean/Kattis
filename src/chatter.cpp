...
inline void solve() {
    ...
    while (cin >> n >> r >> a >> b >> c) {
        ...
        hmap<int, int> mp;
        rep(i, 0, n)mp[uf.find(i)] = uf.setsize(i);
        map<int, int> cnt;
        int res = 0;
        vs ans;
        for (const auto &[a, b]: mp)cnt[b]++;
        for (auto it = cnt.rbegin(); it != cnt.rend(); it++) {
            if (it->second == 1)ans.pb(to_string(it->first));
            else ans.pb(to_string(it->first) + "x" + to_string(it->second));
            res += it->second;
        }
        pr(res);
        pr(ans);
