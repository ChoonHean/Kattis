...
inline void solve() {
    ...
    vs a(n);
    reada(a);
    for (const int &i: chg)a[0][i] = '~';
    vvb visit(n, vb(m));
    int res = 0;
    queue<pii> q;
    auto enq = [&](int x, int y) {
        if (x < 0 || x >= n || y < 0 || y >= m)return;
        if (a[x][y] != '#' && a[x][y] != '?' && !visit[x][y])visit[x][y] = 1, q.emplace(x, y), a[x][y] = '~';
    };
    rep(i, 0, n)
        rep(j, 0, m)
            if (a[i][j] == '~' && !visit[i][j]) {
                ...
                while (!q.empty()) {
                    auto [x, y] = q.front();
                    q.pop();
                    if (x < n - 1 && a[x + 1][y] == 'O')enq(x + 1, y);
                    if (x < n - 1 && (a[x + 1][y] == '#' || a[x + 1][y] == '?'))enq(x, y - 1), enq(x, y + 1);
                }
            }
    for (const string &s: a)pnl(s);
}
