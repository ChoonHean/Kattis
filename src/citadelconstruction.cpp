...
inline void solve() {
    ...
    n = sz(arr);
    if (n <= 4) {
        int res = polygonArea2(arr);
        pnl(to_string(res >> 1) + (res & 1 ? ".5" : ""));
        return;
    }
    int res = 0;
    auto f = [&](int i, int j, int a) -> double {
        return fabs(lineDist(arr[i], arr[j], arr[a]));
    };
    rep(i, 0, n)
        rep(j, i + 2, n) {
            int lo1 = i + 1, hi1 = j - 1;
            while (hi1 - lo1 > 4) {
                int a = lo1 + hi1 >> 1, b = a + 1;
                if (f(i, j, a) > f(i, j, b))hi1 = b;
                else lo1 = a;
            }
            int lo2 = j + 1, hi2 = i - 1 + n;
            while (hi2 - lo2 > 4) {
                int a = lo2 + hi2 >> 1, b = a + 1;
                if (f(i, j, a % n) > f(i, j, b % n))hi2 = b;
                else lo2 = a;
            }
            hi2 = min(hi2, n - 1);
            rep(k, lo1, hi1 + 1)
                rep(l, lo2, hi2 + 1) {
                    res = max(res, (int) (arr[i].cross(arr[k]) + arr[k].cross(arr[j]) + arr[j].cross(arr[l]) +
                                          arr[l].cross(arr[i])));
                }
        }
    pnl(to_string(res >> 1) + (res & 1 ? ".5" : ""));
