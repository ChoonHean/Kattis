...
template<typename T, typename U>
inline vector<int> dijkstra(const vector<vector<pair<T, U>>> &adj, int s) {
    int n = sz(adj);
...
inline void solve() {
    ...
    vpii a;
    auto d = [](int a, int b) { return a * a + b * b; };
    rep(i, 0, n) {
        cin >> x >> y;
        a.eb(x, y);
        rep(j, 0, i)adj[j].eb(i, d(x - a[j].first, y - a[j].second)), adj[i].eb(j, adj[j].back().second);
    }
    vi p = dijkstra(adj, n - 2);
    vi res;
    int cur = p[n - 1];
    while (cur != n - 2) {
        res.pb(cur);
    ...
    if (sz(res))reverse(all(res)), pr(res);
    else cout << '-';
}
