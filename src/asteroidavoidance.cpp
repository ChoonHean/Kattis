...
inline void solve() {
    ...
    for (auto &[x, y]: a)cin >> x.x >> x.y >> y;
    rep(i, 0, m) {
        cin >> x >> y;
        bool ok = 1;
        const P b(x, y), c(x + w, y), d(x, y + h), e(x + w, y + h);
        vector<P> ps{b, c, e, d};
        for (const auto &[p, r]: a) {
            ok = !inPolygon(ps, p, 0);
            if (segDist(b, c, p) <= r)ok = 0;
            if (segDist(b, d, p) <= r)ok = 0;
            if (segDist(c, e, p) <= r)ok = 0;
            if (segDist(d, e, p) <= r)ok = 0;
            if (!ok)break;
        }
        pr("DOOMSLUG", ok ? "GO!" : "STOP!");
    }
}
