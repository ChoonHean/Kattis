...
inline void solve() {
    ...
    rep(i, 0, n) {
        cin >> a[i] >> d;
        while (d--)cin >> u, adj[i].pb(u - 1), deg[u - 1]++;
    }
    PQ<pii, vpii, greater<>> pq;
    rep(i, 0, n)if (!deg[i])pq.emplace(a[i], i);
    vi ord;
    while (!pq.empty()) {
        auto [t, i] = pq.top();
        ord.pb(i);
        pq.pop();
        for (const int &j: adj[i])if (!--deg[j])pq.emplace(a[j], j);
    }
    int res = 0;
    rep(i, 0, n)res = max(res, a[ord[i]] + n - i - 1);
    cout << res;
}
