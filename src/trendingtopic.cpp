...
inline void solve() {
    ...
    vs cur;
    while (cin >> s) {
        if (isalpha(s[0])) { if (sz(s) >= 4)mp[s]++, cur.pb(s); }
        else if (s[1] == '/') {
            q.push(cur);
            cur.clear();
            if (sz(q) == 8) {
                for (const string &t: q.front())mp[t]--;
                q.pop();
            }
        } else if (isdigit(s[0])) {
            int n = stoi(s);
            PQ<int, vi, greater<>> pq;
            ...
            for (const auto &[a, b]: mp)if (b >= mn)res.eb(-b, a);
            sort(all(res));
            cout << "<top " << n << ">\n";
            for (const auto &[a, b]: res)pr(b, -a);
            cout << "</top>\n";
        }
    }
}
