...
inline void solve() {
    int x1,y1,x2,y2,n;
    cin>>x1>>y1>>x2>>y2>>n;
    vector<pair<string,P>>arr(n);
    for(auto&[x,y]:arr)cin>>x>>y.x>>y.y;
    double mn=inf;
    vs res;
    P a(x1,y1),b(x2,y2);
    for(auto&[city,loc]:arr){
        double dist=fabs(lineDist(a,b,loc));
        if(fabs(dist-mn)<eps)res.pb(city);
        else if(dist<mn)res={city},mn=dist;
    }
    pr(res);
}
