...
inline void solve() {
    string a, b, c;
    cin >> a >> b >> c;
    int t = stoi(a.substr(0, a.find('.'))) * 100 + stoi(a.substr(a.find('.') + 1));
    int p1 = stoi(b.substr(0, b.find('.'))) * 100 + stoi(b.substr(b.find('.') + 1));
    int p2 = stoi(c.substr(0, c.find('.'))) * 100 + stoi(c.substr(c.find('.') + 1));
    bool ans = 0;
    rep(i, 0, t + 1) {
        int r = t - i * p1;
        if (r < 0)break;
        if (r % p2 == 0)ans = 1, pr(i, r / p2);
    }
    if (!ans)cout << "none";
}
