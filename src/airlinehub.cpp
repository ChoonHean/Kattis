...
inline void solve() {
    ...
    while (cin >> n) {
        ...
        readpair(a);
        vpdd arr;
        for (auto &[i, j]: a)arr.eb(i * M_PI / 180.0, j * M_PI / 180.0);
        double mn = inf;
        int res = 0;
        rep(i, 0, n) {
            double mx = 0;
            rep(j, 0, n) {
                mx = max(mx, sphericalDistance(arr[j].second, arr[j].first, arr[i].second, arr[i].first, 1));
            }
            if (mx <= mn) {
                mn = mx;
                res = i;
