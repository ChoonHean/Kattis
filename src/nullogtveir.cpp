...
inline void solve() {
    ...
    int n = sz(s);
    vvl dp(n, vl(2, -1));
    auto f = [&](auto &self, int i, bool ishigh) -> ll {
        if (i == n)return 1;
        if (dp[i][ishigh] != -1)return dp[i][ishigh];
        if (ishigh) {
            if (s[i] == '0')return dp[i][1] = self(self, i + 1, 1);
            if (s[i] == '1')return dp[i][1] = self(self, i + 1, 0);
            if (s[i] == '2')return dp[i][1] = self(self, i + 1, 0) + self(self, i + 1, 1);
            return dp[i][1] = 2 * self(self, i + 1, 0);
        }
        return dp[i][0] = 2 * self(self, i + 1, 0);
    };
    cout << f(f, 0, 1);
}
