...
inline void solve() {
    ...
    rep(i, 0, n) {
        ...
        while (m--) {
            ...
            int x;
            auto it = mp1.find(s);
            if (it == mp1.end())x = mp1[s] = cnt1++, a.eb(0), w1.pb(s);
            else x = it->second;
            a[x] |= 1 << i;
        }
        cin >> m;
        while (m--) {
            ...
            int x;
            auto it = mp2.find(s);
            if (it == mp2.end())x = mp2[s] = cnt2++, b.eb(0), w2.pb(s);
            else x = it->second;
            b[x] |= 1 << i;
        }
    }
    vector<pair<string, string>> res;
    rep(i, 0, cnt1)rep(j, 0, cnt2)if (a[i] == b[j])res.eb(w1[i], w2[j]);
    sort(all(res));
    for (const auto &[x, y]: res)cout << '(' << x << ", " << y << ")\n";
    cout << nl;
}
