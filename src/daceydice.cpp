...
inline void solve() {
    ...
    vs a(n);
    reada(a);
    vector<vvb> visit(n, vvb(n, vb(6)));
    queue<ti> q;
    rep(i, 0, n)rep(j, 0, n)if (a[i][j] == 'S')q.emplace(i, j, 2), visit[i][j][2] = 1;
    //0 top, 1 bottom, 2 left, 3 up, 4 right, 5 down
    while (!q.empty()) {
        auto [i, j, k] = q.front();
        int l;
        q.pop();
        if (i) {
            ...
            if (a[i - 1][j] != '*' && !visit[i - 1][j][l])q.emplace(i - 1, j, l), visit[i - 1][j][l] = 1;
        }
        if (i < n - 1) {
            if (k == 3)l = 0;
            else if (k == 5)l = 1;
        ...
        if (j) {
            ...
            if (a[i][j - 1] != '*' && !visit[i][j - 1][l])q.emplace(i, j - 1, l), visit[i][j - 1][l] = 1;
        }
        if (j < n - 1) {
            if (k == 2)l = 0;
            else if (k == 4)l = 1;
