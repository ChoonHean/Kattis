...
inline void solve() {
    ...
    char a, b;
    int u, v;
    vvi lovecat(c + 1), hatecat(c + 1), lovedog(d + 1), hatedog(d + 1);
    vvi g(n);
    rep(i, 0, n) {
        scanf("%c%d %c%d\n", &a, &u, &b, &v);
        if (a == 'C') {
            for (const int &j: hatecat[u])g[i].pb(j);
            lovecat[u].pb(i);
        } else {
            for (const int &j: hatedog[u])g[j].pb(i);
            lovedog[u].pb(i);
        }
        if (b == 'C') {
            for (const int &j: lovecat[v])g[j].pb(i);
            hatecat[v].pb(i);
        } else {
            for (const int &j: lovedog[v])g[i].pb(j);
            hatedog[v].pb(i);
        }
    }
    vi btoa(n, -1);
    pnl(n - hopcroftKarp(g, btoa));
}
