...
inline void solve() {
    ...
    rep(i, 0, m) {
        ...
        rep(j, 0, 13) {
            if (j + w > 12) {
                a2[u + j * n].eb(v + w * n, 24 - j + w);
                a2[v + j * n].eb(u + w * n, 24 - j + w);
            } else {
                a2[u + j * n].eb(v + (j + w) * n, w);
                a2[v + j * n].eb(u + (j + w) * n, w);
            }
        }
    }
    int sp = dijkstra(a1, 0)[n - 1];
    if (sp % 12)sp += sp - sp % 12;
    else sp += sp - 12;
    if (sp < 0)sp = 0;
    int res = inf;
    vi dist = dijkstra(a2, 0);
    rep(j, 0, 13)res = min(res, dist[n - 1 + j * n]);
    cout << res - sp << nl;
}
