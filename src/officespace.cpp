...
inline void solve() {
    ...
    while (cin >> w) {
        cin >> h >> n;
        vector<vector<pbb>> seat(h, vector<pbb>(w));
        vector<pair<string, t>> arr(n);
        for (auto &[s, t]: arr) {
            auto &[a, b, c, d] = t;
            cin >> s >> a >> b >> c >> d;
            rep(i, b, d)rep(j, a, c)seat[i][j].second = seat[i][j].first, seat[i][j].first = 1;
        }
        pr("Total", w * h);
        int res = 0, contest = 0;
        rep(i, 0, h)rep(j, 0, w)res += !seat[i][j].first, contest += seat[i][j].second;
        pr("Unallocated", res);
        pr("Contested", contest);
        for (auto &[s, t]: arr) {
            auto &[a, b, c, d] = t;
            res = 0;
            rep(i, b, d)rep(j, a, c)res += !seat[i][j].second;
            pr(s, res);
        }
        cout << nl;
    }
