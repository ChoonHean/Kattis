...
inline void solve() {
    ...
    rep(i, 0, b)
        rep(j, 0, r)
            dist[i][j] = hypot(arr[i].first - brr[j].first, arr[i].second - brr[j].second), d.pb(dist[i][j]);
    sort(all(d));
    int lo = 0, hi = sz(d) - 1;
    vvi adj(b);
    vi btoa;
    while (lo < hi) {
        int mid = lo + hi + 1 >> 1;
        double lim = d[mid];
        rep(i, 0, b)adj[i].clear();
        rep(i, 0, b)rep(j, 0, r)if (dist[i][j] < lim)adj[i].pb(j);
        btoa.assign(r, -1);
        int mis = tot - hopcroftKarp(adj, btoa);
        if (mis >= n)lo = mid;
        else hi = mid - 1;
    }
    cout << d[lo];
