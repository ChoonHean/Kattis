...
struct hash1 {
    ull operator()(const P &p) const {
        return p.x | ((ll) p.y << 32);
    }
};
...
inline void solve() {
    ...
    for (auto &[x, y]: arr)cin >> x.x >> x.y >> y.x >> y.y;
    hmap<P, hmap<double, int>, hash1> mp;
    rep(i, 0, n) {
        P cur = arr[i].first - arr[i].second;
        int g = gcd(cur.x, cur.y);
        cur.x /= g;
        cur.y /= g;
        if (cur.x < 0)cur.x = -cur.x, cur.y = -cur.y;
        if (cur.x == 0)cur.y = abs(cur.y);
        rep(j, i + 1, n) {
            P vec = arr[j].first - arr[j].second;
            if (cur.cross(vec) == 0)mp[cur][fabs((double) cur.cross(arr[j].first - arr[i].first) / cur.dist2())]++;
        }
    }
    ll res = 0;
    for (const auto &[vec, h]: mp) {
        P cur = vec.y > 0 ? P(vec.y, -vec.x) : P(-vec.y, vec.x);
        if (cur.x == 0)cur.y = abs(cur.y);
        auto it = mp.find(cur);
        if (it == mp.end())continue;
        auto m = it->second;
        for (const auto &[dist, cnt]: h) {
            auto it2 = m.find(dist);
            if (it2 != m.end())res += (ll) cnt * it2->second;
        }
    }
    for (auto &[a, b]: mp)pr(a, b);
    cout << res / 2;
}
