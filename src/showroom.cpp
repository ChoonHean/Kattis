...
template<typename T>
inline vector<T> dijkstra(const vector<vector<pair<int, T>>> &adj, int s, int t) {
    int n = sz(adj);
    ...
    rep(i, 0, n)
        rep(j, 0, m) {
            ...
            rep(k, 0, 4) {
                ...
                if (x >= 0 && x < n && y >= 0 && y < m && a[x][y] != '#')adj[i * m + j].eb(x * m + y, a[x][y] == 'c');
            }
        }
    rep(i, 0, n) {
        if (a[i][0] == 'D')adj[i * m].eb(N, 0);
        if (a[i][m - 1] == 'D')adj[i * m + m - 1].eb(N, 0);
    }
    rep(j, 1, m - 1) {
        if (a[0][j] == 'D')adj[j].eb(N, 0);
        if (a[n - 1][j] == 'D')adj[(n - 1) * m + j].eb(N, 0);
    }
    int x, y;
    cin >> x >> y;
    cout << 1 + dijkstra(adj, (x - 1) * m + y - 1, N)[N];
}
