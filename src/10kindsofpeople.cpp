...
inline void solve() {
    ...
    vvi cc(n, vi(m));
    int cnt = 0;
    auto enq = [&](const int &x, const int &y, const int &nx, const int &ny) {
        if (nx < 0 || nx >= n || ny < 0 || ny >= m)return;
        if (a[x][y] == a[nx][ny] && !visit[nx][ny])visit[nx][ny] = 1, q.emplace(nx, ny);
    };
    rep(i, 0, n) {
        rep(j, 0, m)
            if (!visit[i][j]) {
    ...
    while (qu--) {
        cin >> r1 >> c1 >> r2 >> c2;
        pnl(cc[r1 - 1][c1 - 1] == cc[r2 - 1][c2 - 1] ? a[r1 - 1][c1 - 1] == '1' ? "decimal" : "binary" : "neither");
    }
