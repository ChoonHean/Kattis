...
inline void solve() {
    ...
    int N = 1 << s;
    vvl dp(s, vl(N, llinf));
    rep(j, 0, s)dp[j][1 << j] = dist[j][0];
    rep(i, 0, N)
        rep(j, 0, s) {
            rep(k, 0, s)
                if (!((i >> k) & 1)) {
                    int next = i | (1 << k);
                    dp[k][next] = min(dp[k][next], dp[j][i] + dist[j][arr[k]]);
                }
        }
    ll res = llinf;
    rep(i, 0, s)res = min(res, dp[i].back() + dist[i][0]);
    cout << res;
}
