...
inline void solve() {
    ...
    cin >> n;
    hset<string> dict;
    rep(i, 0, n)cin >> s, dict.insert(s);
    while (cin >> s) {
        if (dict.contains(s))continue;
        vi pos;
        rep(i, 0, sz(s))if (isdigit(s[i]))pos.pb(i);
        if (sz(pos) > 3) {
            pnl(s);
            continue;
        }
        ...
        for (const int &i: pos) {
            ...
            while (size--) {
                ...
                for (char c = 'a'; c <= 'z'; c++)t[i] = c, q.push(t);
                for (char c = 'A'; c <= 'Z'; c++)t[i] = c, q.push(t);
            }
        }
        for (int i = sz(pos); i < 3 && ok; i++) {
            int size = sz(q);
            while (size--) {
                string t = q.front();
                q.pop();
