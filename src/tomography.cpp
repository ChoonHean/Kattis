...
inline void solve() {
    ...
    reada(a);
    reada(b);
    int t = accumulate(all(a), 0), t2 = accumulate(all(b), 0);
    if (t != t2) {
        cout << "No";
        return;
    }
    ...
    rep(i, 0, n)rep(j, n, N)mf.add_edge(i, j, 1);
    rep(i, 0, n)mf.add_edge(N, i, a[i]);
    rep(j, n, N)mf.add_edge(j, N + 1, b[j - n]);
    pnl(mf.dinic(N, N + 1) == t ? "Yes" : "No");
}
