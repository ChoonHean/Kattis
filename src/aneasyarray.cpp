...
struct S {
    ...
    S() : mx1(-inf), mx2(-inf), mn1(inf), mn2(inf) {};

    S(int a, int b, int c, int d) : mx1(a), mx2(b), mn1(c), mn2(d) {}
};

S op(const S &s1, const S &s2) {
    int a = s1.mx1, b = s1.mx2, c = s2.mx1, d = s2.mx2;
    if (b > a)swap(a, b);
    if (d > c)swap(d, c);
    if (d > a)a = c, b = d; //c,d,a,b
    else if (d > b) { //a/c,d,b
        if (c > a)swap(a, c);
        b = max(c, d);
    } else if (c > b) { //a,c,b,d
    ...
    int w = s1.mn1, x = s1.mn2, y = s2.mn1, z = s2.mn2;
    if (x < w)swap(x, w);
    if (z < y)swap(z, y);
    if (z < w)w = y, x = z;
    else if (z < x) {
...
inline void solve() {
    ...
    vector<S> arr(n);
    rep(i, 0, n) {
        cin >> arr[i].mx1, arr[i].mn1 = arr[i].mx1;
    }
    segtree<S, op, e> st(arr);
    int l, r;
    while (q--) {
        cin >> l >> r;
        r--;
        S s = st.prod(l, r);
        ll o = arr[l - 1].mx1 * arr[r].mx1;
        pnl(max(o * s.mx1 * s.mx2, max(o * s.mn1 * s.mn2, o * s.mx1 * s.mn1)));
    }
}
