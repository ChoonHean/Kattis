...
inline void solve() {
    ...
    rep(k, 0, h) {
        cin >> x;
        int add1 = N * (k << 1), add2 = N * (k << 1 ^ 1), add3 = N * (k + 1 << 1);
        rep(i, 0, n)
            rep(j, 0, n) {
                if (arr[i][j] > prev) {
                    int from = add1 + i * n + j;
                    if (i)mf.add_edge(from, add2 + (i - 1) * n + j, 1);
                    if (i < n - 1)mf.add_edge(from, add2 + (i + 1) * n + j, 1);
                    if (j)mf.add_edge(from, add2 + i * n + j - 1, 1);
                    if (j < n - 1)mf.add_edge(from, add2 + i * n + j + 1, 1);
                    mf.add_edge(from, add2 + i * n + j, 1);
                }
                if (arr[i][j] > x) {
                    mf.add_edge(add2 + i * n + j, add3 + i * n + j, 1);
                }
            }
        prev = x;
    }
    for (auto &[i, j]: start)mf.add_edge(M, i * n + j, 1);
    rep(i, 0, N)mf.add_edge((N * (h << 1)) + i, M + 1, 1);
    cout << mf.dinic(M, M + 1);
}
