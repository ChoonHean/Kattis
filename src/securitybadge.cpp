...
inline void solve() {
    ...
    for (const int &t: uniq) {
        ...
        while (!q.empty()) {
            int i = q.front();
            q.pop();
            for (const auto &[j, x, y]: adj[i]) {
                if (!(x <= t && t <= y))continue;
                if (j == d) {
                    visit[j] = 1;
                    break;
                }
                if (!visit[j])visit[j] = 1, q.push(j);
            }
        }
        if (visit[d])res.eb(t, 1);
        else res.eb(t, 0);
    }
    sort(all(res));
    int ans = 0;
    rep(i, 0, sz(res) - 1)if (res[i].second)ans += res[i + 1].first - res[i].first;
    cout << ans;
}
