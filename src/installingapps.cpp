...
inline void solve() {
    ...
    rep(i, 0, n)cin >> a[i].first.first >> a[i].first.second, a[i].second = i + 1;
    sort(all(a), [](const auto &a, const auto &b) {
        int c = a.first.second - a.first.first;
        int d = b.first.second - b.first.first;
        return c < d;
    });
    vvpii dp(n + 1, vpii(n + 1, {inf, inf}));
    dp[0][0] = {0, -1};
    rep(i, 0, n) {
        rep(k, 0, i + 1) {
            int need = dp[i][k].first + max(a[i].first.first, a[i].first.second);
            if (need > c)dp[i + 1][k + 1] = dp[i][k + 1];
            else dp[i + 1][k + 1] = min(dp[i][k + 1], {dp[i][k].first + a[i].first.second, i});
        }
    }
    int res = 0;
    repr(i, n, 0)
    ...
    pnl(res);
    int cur = dp[n][res].second;
    vi ans;
    while (res)ans.pb(a[cur].second), cur = dp[cur][--res].second;
    reverse(all(ans));
    pr(ans);
}
