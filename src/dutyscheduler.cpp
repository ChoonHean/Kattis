...
inline void solve() {
    ...
    for (; res <= n; res++) {
        ...
        rep(i, 0, m)for (const int &j: a[i])mf.add_edge(i, j + m, 1);
        rep(i, m, N)mf.add_edge(i, N + 1, 2);
        rep(i, 0, m)mf.add_edge(N, i, res);
        if (mf.dinic(N, N + 1) == n << 1)break;
    }
    printf("%d\n", res);
    vvi ans(n);
    rep(i, 0, m)
        for (const int &j: mf.AL[i]) {
            ...
            if (flow > 0)ans[v - m].pb(i);
        }
    rep(i, 0, n)printf("Day %d: %s %s\n", i + 1, name[ans[i][0]].c_str(), name[ans[i][1]].c_str());
}
