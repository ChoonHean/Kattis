...
inline void solve() {
    ...
    cin >> n;
    vector<pair<P, P>> a(n);
    for (auto &[b, c]: a)cin >> b.x >> b.y >> c.x >> c.y;
    cin >> q;
    double x1, y1, x2, y2;
    while (q--) {
        cin >> x1 >> y1 >> x2 >> y2;
        ...
        rep(i, 0, n) {
            P p1(x1, y1), p2(x2, y2);
            auto ret = lineInter(p1, p2, a[i].first, a[i].second);
            if (ret.first == 1)cnt += segDist(p1, p2, ret.second) < eps;
        }
        pnl(cnt & 1 ? "different" : "same");
    }
}
