...
inline void solve() {
    int n, m, k;
    cin >> n >> k >> m;
    hmap<string, bool> cache;
    string s;
    vvi arr(n, vi(26));
    rep(0, n) {
        cin >> s;
        for (char &c: s)arr[i][c - 'A'] = 1;
    }
    auto f = [&](auto &self, int n, string s) -> bool {
        ...
        rep(0, sz(s)) {
            ...
            if (seen[j])continue;
            seen[j] = 1;
            if (arr[n][j])if (self(self, n + 1, s.substr(0, i) + s.substr(i + 1)))return cache[s] = true;
        }
