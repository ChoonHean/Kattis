...
inline void solve() {
    ...
    rep(i, 0, n)
        rep(j, 0, m) {
            if (i && a[i - 1][j] == a[i][j])uf.unionset((i - 1) * m + j, i * m + j);
            if (j && a[i][j - 1] == a[i][j])uf.unionset(i * m + j - 1, i * m + j);
        }
    rep(i, 0, n)
        rep(j, 0, m) {
            if (i && a[i - 1][j] < a[i][j])adj[uf.find(i * m + j)].pb(uf.find((i - 1) * m + j));
            if (i < n - 1 && a[i + 1][j] < a[i][j])adj[uf.find(i * m + j)].pb(uf.find((i + 1) * m + j));
            if (j && a[i][j - 1] < a[i][j])adj[uf.find(i * m + j)].pb(uf.find(i * m + j - 1));
            if (j < m - 1 && a[i][j + 1] < a[i][j])adj[uf.find(i * m + j)].pb(uf.find(i * m + j + 1));
        }
    int res = 0;
    rep(i, 0, n)rep(j, 0, m)res += adj[uf.find(i * m + j)].empty();
    cout << res;
}
