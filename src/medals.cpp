...
inline void solve() {
    int g = -1, s = -1, b = -1, x, y, z;
    string name;
    vti a;
    int t = 0;
    rep(i, 0, n) {
        cin >> name >> x >> y >> z;
        if (name == "Canada")g = x, s = y, b = z;
        else a.eb(x, y, z);
        t += x + y + z;
    }
    if (g == -1) {
        pnl("Canada cannot win.");
        return;
    }
    vl p{1};
    rep(i, 0, 5)p.pb(p.back() * t);
    a.insert(a.begin(), {g, s, b});
    rep(i, 0, 5)
        rep(j, 0, 5)
            rep(k, 0, 5) {
                vti cur(a);
                stable_sort(all(cur), [&](ti a, ti b) {
                    return get<0>(a) * p[j] * p[k] + get<1>(a) * p[i] * p[k] + get<2>(a) * p[j] * p[i] >=
                           get<0>(b) * p[j] * p[k] + get<1>(b) * p[i] * p[k] + get<2>(b) * p[j] * p[i];
                });
                if (cur[0] == make_tuple(g, s, b)) {
                    pnl("Canada wins!");
                    return;
                }
            }
