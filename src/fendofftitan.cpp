...
struct T {
    ...
    T operator+(const T &other) const {
        return T(a + other.a, b + other.b, c + other.c);
    }
};

#define infT T{inf,inf,inf}

template<typename T>
inline vector<T> dijkstra(const vector<vector<pair<int, T>>> &adj, int s) {
    int n = sz(adj);
//    vector<T> dist(adj.size(), is_same<T, ll>() ? llinf : inf);
    vector<T> dist(n, infT);
    dist[s] = T();
    set<pair<T, int>> pq;
    pq.emplace(T(), s);
//    vi p(n, -1);
...
inline void solve() {
    ...
    rep(i, 0, m) {
        ...
        u--;
        v--;
        adj[u].emplace_back(v, T(c == 2 ? 1 : 0, c == 1 ? 1 : 0, w));
        adj[v].emplace_back(u, T(c == 2 ? 1 : 0, c == 1 ? 1 : 0, w));
    }
    vector<T> d = dijkstra(adj, s - 1);
    T res = d[t - 1];
    if (res.a == inf)cout << "IMPOSSIBLE" << nl;
    else pr(res.c, res.b, res.a);
}
