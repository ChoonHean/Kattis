...
struct Hash {
    size_t operator()(const ti &t) const {
        return get<0>(t) | (get<1>(t) << 10) | (get<2>(t) << 20);
    }
};
...
inline void solve() {
    ...
    cin >> n;
    hmap<ti, hset<ti, Hash>, Hash> a;
    hmap<pii, hset<pii, Hashp>, Hashp> b;
    rep(i, 0, n) {
        cin >> x1 >> y1 >> z1 >> x2 >> y2 >> z2;
        ti u{x1, y1, z1}, v{x2, y2, z2};
        pii y{x1, y1}, z{x2, y2};
        a[u].insert(v);
        a[v].insert(u);
        if (y != z)b[y].insert(z), b[z].insert(y);
    }
    bool ok1 = 1, ok2 = 1;
    hset<ti, Hash> vis1;
    hset<pii, Hashp> vis2;
    auto f = [&](auto &self, const ti &i, const ti &p) -> void {
        for (const ti &j: a[i]) {
            if (j == p)continue;
            if (!vis1.insert(j).second) {
                ok1 = 0;
                return;
            }
            self(self, j, i);
    ...
    auto f2 = [&](auto &self, const pii &i, const pii &p) -> void {
        for (const pii &j: b[i]) {
            if (j == p)continue;
            if (!vis2.insert(j).second) {
                ok2 = 0;
                return;
            }
            self(self, j, i);
    ...
    for (const auto &[x, y]: a)if (vis1.insert(x).second)f(f, x, x);
    for (const auto &[x, y]: b)if (vis2.insert(x).second)f2(f2, x, x);
    if (ok1)pnl("No true closed chains");
    else pnl("True closed chains");
    if (ok2)cout << "No floor closed chains";
