...
inline void solve() {
    ...
    int m = 1 << k;
    ordered_set have;
    rep(i, 0, n)have.insert(s[i]);
    vi a;
    rep(i, 0, n)a.pb(have.order_of_key(s[i]));
    vector<hmap<int, int>> cnts(k);
    rep(i, 0, k) {
        bool first = 0;
        int cur = 0, j = 0;
        while (!first)first = a[j++] == i;
        for (; j < n; j++) {
            if (a[j] == i)cnts[i][cur]++, cur = 0;
            else cur |= 1 << a[j];
        }
        cnts[i].erase(0);
    }
    ...
    rep(i, 0, m) {
        rep(j, 0, k)
            if (!((i >> j) & 1)) {
                int c = 1;
                for (const auto &[x, y]: cnts[j])if ((x | i) != i)c += y;
