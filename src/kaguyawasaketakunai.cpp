...
inline void solve() {
    ...
    max_flow mf(n);
    vl dist = dijkstra(adj, src), dist2 = dijkstra(adj, dst);
    ll sp = dist[dst];
    rep(i, 0, n)for (auto &[j, w]: adj[i])if (dist[i] + dist2[j] + w == sp)mf.add_edge(i, j, 1);
    mf.dinic(src, dst);
    vpii res = mf.min_cut(src, dst);
    pnl(sz(res));
    for (auto &[a, b]: res)pr(a + 1, b + 1, 1);
}
