...
inline void solve() {
    const vs name{"Twilight Sparkle", "Applejack", "Rarity", "Pinkie Pie", "Fluttershy", "Rainbow Dash", "Spike"};
    vvi a(7, vi(7));
    max_flow mf(16);
    int op = 1;
    read2d(a);
    for (; op <= 7; op++) {
        mf.EL.clear();
        for (auto &v: mf.AL)v.clear();
        rep(i, 0, 7)rep(j, 0, 7)if (a[i][j])mf.add_edge(i, j + 7, 1);
        rep(i, 0, 7)mf.add_edge(i + 7, 15, 1), mf.add_edge(14, i, op);
        if (mf.dinic(14, 15) == 7)break;
    }
    if (op == 8)cout << "IMPOSSIBLE";
    else {
        vector<vector<psi>> res(op);
        rep(i, 0, 7) {
            int p = 0;
            for (const int &j: mf.AL[i]) {
                const auto &[v, cap, flow] = mf.EL[j];
                if (flow > 0)res[p++].eb(name[i], v - 6);
            }
        }
        pnl(op);
        for (const auto &v: res) {
            pnl(sz(v));
            for (const auto &[x, y]: v)pr(x, y);
        }
    }
}
