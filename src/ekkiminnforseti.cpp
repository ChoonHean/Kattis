...
// Why does this fail the 3rd last test case
inline void solve() {
    int m, n, a;
    cin >> n >> m;
    vs names(m);
    ...
    while (sz(votes) > 1) {
        ...
        votes.erase(idx);
        in[idx] = 0;
        for (int i: supporters[idx]) {
            while (!in[arr[i].front()])arr[i].pop();
            votes[arr[i].front()]++;
            supporters[arr[i].front()].pb(i);
        }
    }
    cout << names[votes.begin()->first];
}
