...
inline void solve() {
    //2*q*n queue at each downlink window
    //n downlink windows
    int n, q, s, u, v, w, t = 0;
    cin >> n >> q >> s;
    const int N = 2 * q * n + n;
    ...
    read(s);
    for (int &i: a)i--;
    //queue i at time j -> i*2*n+j*2/i*2*n+j*2+1
    rep(i, 0, q) {
        cin >> w;
        rep(j, 0, n) {
            mf.add_edge(i * 2 * n + j * 2, i * 2 * n + j * 2 + 1, w);
            if (j != n - 1)mf.add_edge(i * 2 * n + j * 2 + 1, i * 2 * n + j * 2 + 2, inf);
            mf.add_edge(i * 2 * n + j * 2 + 1, N - n + j, inf);
        }
    }
    rep(i, 0, n) {
        cin >> w;
        mf.add_edge(N - n + i, N + 1, w);
        rep(j, 0, s) {
            cin >> w;
            t += w;
            mf.add_edge(N, a[j] * 2 * n + i * 2, w);
        }
    }
    cout << (mf.dinic(N, N + 1) == t ? "possible" : "impossible");
}
