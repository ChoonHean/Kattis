...
inline void solve() {
    ...
    rep(i, 0, n) {
        ...
        int u;
        ll v = x + y;
        if (mp.contains(v))u = mp[v];
        else u = mp[v] = cnt++, res.pb(v);
        g[i].pb(u);
        v = x - y;
        if (mp.contains(v))u = mp[v];
        else u = mp[v] = cnt++, res.pb(v);
    ...
    vector<pair<char, ll>> ans(n);
    rep(i, 0, cnt)
        if (btoa[i] != -1) {
            ll u;
            u = ans[btoa[i]].second = res[i];
            auto &[x, y] = a[btoa[i]];
            if (x + y == u)ans[btoa[i]].first = '+';
            else if (x - y == u)ans[btoa[i]].first = '-';
            else ans[btoa[i]].first = '*';
        }
    rep(i, 0, n) {
        const auto &[x, y] = a[i];
        const auto &[c, v] = ans[i];
        pr(x, c, y, '=', v);
    }
}
