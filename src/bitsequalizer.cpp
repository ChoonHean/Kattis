...
struct min_cost_max_flow {
    ...
    void add_edge(int u, int v, int w, int c, bool directed = true) {
...
inline void solve() {
    ...
    int m = sz(b);
    min_cost_max_flow mcmf(m + n + 2);
    rep(i, 0, m) {
        mcmf.add_edge(m + n, i, 1, 0);
        rep(j, 0, n)mcmf.add_edge(i, m + j, 1, (b[i] != j) + (s[j] == '0') + (s[j] != '1' ? 1e5 : 0));
    }
    rep(i, 0, n)mcmf.add_edge(i + m, m + n + 1, 1, 0);
    printf("Case %d: %d\n", tc++, mcmf.mcmf(n + m, n + m + 1).second + q - (m - o) * 100000);
}
