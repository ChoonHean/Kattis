...
inline void solve() {
    ...
    while(!q.empty()){
        ...
        for(int j:adj[i]){
            if(p[j]==-1){
                p[j]=i;
                if(child[i].first==0)child[i].first=j;
                else child[i].second=j;
                q.push(j);
            }
        }
    }
    vi depth(n);
    int res=0;
    auto del=[&](auto&self,int i,int d)->void{
        if(d==0){
            if(child[i].first!=0){
                res++;
                self(self,child[i].first,0);
                child[i].first=0;
            }
            if(child[i].second!=0){
                res++;
                self(self,child[i].second,0);
                child[i].second=0;
            }
        }else{
            if(child[i].first)self(self,child[i].first,d-1);
            if(child[i].second)self(self,child[i].second,d-1);
        }
        depth[i]=d+1;
    };
    for(auto it=topo.rbegin();it!=topo.rend();it++){
        int l=depth[child[*it].first],r=depth[child[*it].second];
        if(l-1>r){
            del(del,child[*it].first,r);
        }else if(r-1>l){
            del(del,child[*it].second,l);
        }
        depth[*it]=max(depth[child[*it].first],depth[child[*it].second])+1;
    }
    pnl(res);
}
