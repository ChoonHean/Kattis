...
inline void solve() {
    ...
    rep(i, 0, n)
        rep(j, 0, m) {
            ...
            else if (a[i][j] == '*')adj[N].eb(i * m + j, 0);
            rep(k, 0, 4) {
                int x = i + dx[k], y = j + dy[k];
                if (x >= 0 && x < n && y >= 0 && y < m && a[x][y] != 'X' && a[x][y] != 'D')
                    adj[i * m + j].eb(x * m + y, 1);
            }
        }
    int res = inf;
    vi d1 = dijkstra(adj, x1 * m + y1), d2 = dijkstra(adj, N);
    rep(k, 0, 4) {
        int x = x2 + dx[k], y = y2 + dy[k];
        if (x >= 0 && x < n && y >= 0 && y < m && d1[x * m + y] < d2[x * m + y])res = min(res, d1[x * m + y]);
    }
