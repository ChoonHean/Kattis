...
inline void solve() {
    ...
    readarr(arr);
    vi ptrs(n);
    hset<string> taken(n * k);
    vector<vs> res(n);
    int ptr = 0;
    rep(j, 0, k)rep(i, 0, n) {
            bool d = 0;
            while (ptrs[i] < sz(pref[i])) {
                if (taken.contains(pref[i][ptrs[i]]))ptrs[i]++;
                else {
                    taken.insert(pref[i][ptrs[i]]);
                    res[i].pb(pref[i][ptrs[i]]);
                    ptrs[i]++;
                    d = 1;
                    break;
                }
            ...
            if (d)continue;
            while (taken.contains(arr[ptr]))ptr++;
            taken.insert(arr[ptr]);
            res[i].pb(arr[ptr++]);
        }
    pr(res);
}
