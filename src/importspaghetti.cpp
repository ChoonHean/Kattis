...
inline void solve() {
    ...
    hmap<string, int> mp;
    vs arr;
    rep(i, 0, n)cin >> s, mp[s] = i, arr.pb(s);
    vvi adj(n, vi(n, inf)), p(n, vi(n, -1));
    rep(i, 0, n) {
        cin >> s >> t;
        getline(cin, s);
        while (t--) {
            ...
            vs a = split(s, ", ");
            a[0] = split(a[0], " ")[1];
            for (const string &ss: a)v = mp[ss], adj[i][v] = 1, p[i][v] = i;
        }
    }
    rep(k, 0, n)
        rep(i, 0, n)
            rep(j, 0, n) {
                if (adj[i][k] + adj[k][j] < adj[i][j])adj[i][j] = adj[i][k] + adj[k][j], p[i][j] = p[k][j];
            }
    int mn = inf, idx;
    rep(i, 0, n)if (adj[i][i] < mn)mn = adj[i][i], idx = i;
    if (mn == inf) {
        cout << "SHIP IT";
        return;
    }
