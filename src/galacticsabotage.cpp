...
S op(S &p, S &q) {
    if (p.mn < q.mn)return p;
    else if (q.mn < p.mn)return q;
    return {p.mn, p.cnt + q.cnt};
}
...
S mp(F f, S s) {
    return S{s.mn + f, s.cnt};
}

F comp(F x, F y) { return x + y; }
...
public:
    ...
    explicit lazy_segtree(const std::vector<S> &v) : _n((ll) (v.size())) {
...
inline void solve() {
    ...
    vector<S> pre;
    pre.pb({arr[0] - 1, 1});
    rep(i, 1, n)pre.pb({pre[i - 1].mn + arr[i] - i - 1, 1});
    lazy_segtree<S, op, e, F, mp, comp, id> st(pre);
    pnl(st.prod(0, n).cnt);
    while (q--) {
        cin >> u >> v;
        if (--v < --u)swap(u, v);
        st.apply(u, v, arr[v] - arr[u]);
        swap(arr[u], arr[v]);
        pnl(st.prod(0, n).cnt);
    }
}
