...
inline void solve() {
    ...
    rep(_, 0, 2) {
        ...
        rep(i, 0, n)
            if (!deg[i]) {
                if (_ & 1)a[i] == 1 ? q1.push(i) : q2.push(i);
                else a[i] == 1 ? q2.push(i) : q1.push(i);
            }
        int cur = 0;
        if (q1.empty())cur++, swap(q1, q2);
        while (!q1.empty()) {
            while (!q1.empty()) {
                int i = q1.front();
                q1.pop();
                for (const int &j: adj[i])
                    if (!--deg[j]) {
                        if (_ & 1)(a[j] & 1) == (cur & 1) ? q2.push(j) : q1.push(j);
                        else (a[j] & 1) == (cur & 1) ? q1.push(j) : q2.push(j);
                    }
            }
            if (q2.empty())break;
            cur++;
