...
inline void solve() {
    ...
    rep(i, 0, n) {
        if (arr[i].contains("Fizz")) {
            if (first == -1)first = i;
            else if (a == -1)a = i - first;
        }
        if (arr[i].contains("Buzz")) {
            if (second == -1)second = i;
            else if (b == -1)b = i - second;
        }
    }
    if (a == -1 && first != -1)a = first + c;
    if (b == -1 && second != -1)b = second + c;
    pr(a == -1 ? d + 1 : a, b == -1 ? d + 1 : b);
}
