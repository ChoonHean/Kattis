...
inline void solve() {
    int r, n, m, xi, yi, xr, yr, x, y;
    cin >> r >> n >> m >> xi >> yi >> xr >> yr;
    vvi dist(n, vi(m, inf));
    queue<pii> q;
    rep(i, 0, r) {
        cin >> x >> y;
    ...
    auto enq = [&](int i, int j, int x, int y) {
        if (i >= 0 && i < n && j >= 0 && j < m) {
            int d = dist[x][y] + 1;
            if (d < dist[i][j]) {
                dist[i][j] = d;
    ...
    int lo = -1, hi = min(dist[xi][yi], dist[xr][yr]);
    while (lo < hi) {
        int mid = lo + hi + 1 >> 1;
        q = {};
        vvi d(n, vi(m, inf));
        d[xi][yi] = 0;
        q.emplace(xi, yi);
        auto en = [&](int i, int j, int x, int y) {
            if (i >= 0 && i < n && j >= 0 && j < m) {
                if (dist[i][j] >= mid) {
                    int cur = d[x][y] + 1;
                    if (cur < d[i][j]) {
        ...
        if (d[xr][yr] != inf)lo = mid, res = d[xr][yr];
        else hi = mid - 1;
    }
    pr(lo, res);
}
