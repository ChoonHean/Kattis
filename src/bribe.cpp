...
inline void solve() {
    ...
    vvd dp(1<<n,vd(c+1,-1));
    auto dfs=[&](auto&self,int mask,int l,int m)->double{
        if(l==0)return 1;
        if(dp[mask][l]!=-1)return dp[mask][l];
        double res=0;
        rep(0,n){
            if(!((mask>>i)&1))if(m>=arr[i]){
                int next=mask|(1<<i);
                res=max(res,prob[i]*self(self,next,l-1,m-arr[i])+(1-prob[i])*self(self,next,l,m-arr[i]));
            }
        }
        return dp[mask][l]=res;
    };
    pnl(dfs(dfs,0,c,m));
}
