...
inline void solve() {
    ...
    rep(i, 1, d + 1) {
        ...
        int mid = tot / 2 + 1;
        if (arr[i].first >= mid)a += arr[i].first - mid, b += arr[i].second, pr('A', arr[i].first - mid, arr[i].second);
        else a += arr[i].first, b += arr[i].second - mid, pr('B', arr[i].first, arr[i].second - mid);
    }
    cout << abs(a - b) / (double) t;
}
