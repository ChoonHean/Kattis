...
inline void solve() {
    ...
    vs arr;
    while (cin >> n) {
        while (sz(arr) < n)arr.pb("");
        getline(cin, s);
        rep(i, 0, n)getline(cin, arr[i]);
        getline(cin, s);
        SuffixArray sa(s);
        int size = sz(s);
        auto cmp = [&](int a, int m, const string &str) -> int {
            rep(i, 0, m) {
        ...
        rep(i, 0, n) {
            ...
            while (lo < hi) {
                int mid = lo + hi >> 1;
                if (cmp(sa.sa[mid], m, arr[i]) < 0)lo = mid + 1;
                else hi = mid;
            }
            int a = lo;
            hi = size;
            ...
            vi res;
            rep(j, a, lo)res.pb(sa.sa[j]);
            if (s.substr(sa.sa[lo], m) == arr[i])res.pb(sa.sa[lo]);
            sort(all(res));
            pr(res);
        }
    }
