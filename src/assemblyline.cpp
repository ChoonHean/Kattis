...
inline void solve() {
    ...
    rep(i, 0, n)
        rep(j, 0, n) {
            cin >> s;
            int pos = s.find('-');
            int part = find(all(a), s.back()) - a.begin();
            adj[part].eb(i, j, stoi(s.substr(0, pos)));
        }
    cin >> q;
    while (q--) {
        cin >> s;
        int m = sz(s);
        vvvi dp(m, vvi(m, vi(n, -1)));
        auto f = [&](auto &self, int i, int j, int k) {
            if (i == j)return s[i] == a[k] ? 0 : inf;
            int &res = dp[i][j][k];
            if (res != -1)return res;
            res = inf;
            for (const auto &[c1, c2, cost]: adj[k]) {
                rep(l, i, j)res = min(res, self(self, i, l, c1) + self(self, l + 1, j, c2) + cost);
            }
            return res;
        };
        int res = inf, id = 0;
