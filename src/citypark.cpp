...
inline void solve() {
    ...
    rep(i, 0, n) {
        ...
        x2 += x1;
        y2 += y1;
        uf.ans[i] = (x2 - x1) * (y2 - y1);
        auto it = h[x1].ub({y1, inf});
        if (it != h[x1].begin())it--;
        while (it != h[x1].end() && it->first.second < y1)it++;
        while (it != h[x1].end() && it->first.first <= y2) {
            if (!uf.sameset(i, it->second))uf.unionset(i, it->second);
            it++;
        }
        it = h[x2].ub({y1, inf});
        if (it != h[x2].begin())it--;
        while (it != h[x2].end() && it->first.second < y1)it++;
        while (it != h[x2].end() && it->first.first <= y2) {
            if (!uf.sameset(i, it->second))uf.unionset(i, it->second);
            it++;
        }
        it = v[y1].ub({x1, inf});
        if (it != v[y1].begin())it--;
        while (it != v[y1].end() && it->first.second < x1)it++;
        while (it != v[y1].end() && it->first.first <= x2) {
            if (!uf.sameset(i, it->second))uf.unionset(i, it->second);
            it++;
        }
        it = v[y2].ub({x1, inf});
        if (it != v[y2].begin())it--;
        while (it != v[y2].end() && it->first.second < x1)it++;
        while (it != v[y2].end() && it->first.first <= x2) {
            if (!uf.sameset(i, it->second))uf.unionset(i, it->second);
            it++;
        }
        h[x1].emplace(pair(y1, y2), i);
        h[x2].emplace(pair(y1, y2), i);
        v[y1].emplace(pair(x1, x2), i);
        v[y2].emplace(pair(x1, x2), i);
    }
    vb vis(n);
    ll res = 0;
    rep(i, 0, n) {
